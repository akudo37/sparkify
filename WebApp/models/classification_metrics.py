...
def calculate_classification_metrics(model_name, df_test_for_model,
                                     output=True):
    '''
    INPUT:
...
def plot_precision_recall_curve(df, title_addition=None, ax=None):
    '''
    INPUT:
    df - (pyspark dataframe) dataset transformed by model, including 'label',
    ...
    In case ax is privided, plt.show() will not be called inside this function.
