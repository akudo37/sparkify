...
def create_preprocess_pipeline():
    ...
    feature_labels - (list) string labels for corresponding feature vector
        elements
...
def main():
    ...
    save_path - (string, mandatory) filepath of parquet file to save cleaned
        and transformed data to
    pipeline_save_path - (string, optional) folderpath of pipeline to save fit
        pipeline to
    pipeline_load_path - (string, optional) folderpath of pipeline to load fit
        pipeline from
        *Note: pipeline_save_path and pipeline_load_path cannot be set at the
            same time
    ...
    parser.add_argument("save_path", help="filepath of parquet file to save \
                        cleaned and transformed data to")

    # optional: either save or load fit pipeline
    group = parser.add_mutually_exclusive_group()
    group.add_argument("-ps", "--pipeline_save_path", type=str,
                       help="folderpath of pipeline to save fit pipeline to")
    group.add_argument("-pl", "--pipeline_load_path", type=str,
